...
def import_to_global(servers, source_client):
    ...
    console.print(f"[cyan]Importing to global configuration...[/]")
    ...
    for server_name, server_config in servers.items():
        ...
        table.add_row(
            server_name,
            command_str[:50] + "..." if len(command_str) > 50 else command_str,
            "Ready to import"
        )
...
def import_client(client_name, profile, list_clients):
    ...
    if success:
        ...
        if profile:
            console.print(f"[dim]Run 'mcpm profile ls' to see the updated profile[/]")
            console.print(f"[dim]Run 'mcpm profile run {profile}' to test the imported servers[/]")
        else:
            console.print(f"[dim]Run 'mcpm ls' to see imported servers[/]")
    else:
        console.print()
