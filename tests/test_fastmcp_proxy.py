...
from mcpm.core.schema import STDIOServerConfig, RemoteServerConfig, CustomServerConfig
from mcpm.fastmcp_integration.proxy import MCPMProxyFactory, create_mcpm_proxy
...
class TestFastMCPProxy:
    ...
    async def test_proxy_stdio_server(self):
        ...
        with patch('mcpm.fastmcp_integration.proxy.FastMCP') as mock_fastmcp:
            ...
            proxy = await factory.create_proxy_for_servers([server], "test")
    ...
    async def test_proxy_mixed_servers(self):
        ...
        with patch('mcpm.fastmcp_integration.proxy.FastMCP') as mock_fastmcp:
            ...
            proxy = await factory.create_proxy_for_servers(servers, "mixed")
    ...
    async def test_proxy_auth_middleware_stdio_mode(self):
        ...
        with patch('mcpm.fastmcp_integration.proxy.FastMCP') as mock_fastmcp:
            ...
            proxy = await factory.create_proxy_for_servers([server], stdio_mode=True)
            
            # Check that auth middleware was NOT added
            middleware_types = [
                type(call[0][0]).__name__ 
                for call in mock_proxy.add_middleware.call_args_list
            ]
    ...
    async def test_proxy_auth_middleware_http_mode(self):
        ...
        with patch('mcpm.fastmcp_integration.proxy.FastMCP') as mock_fastmcp:
            ...
            proxy = await factory.create_proxy_for_servers([server], stdio_mode=False)
